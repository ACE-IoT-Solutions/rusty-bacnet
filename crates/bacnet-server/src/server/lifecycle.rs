...
impl<T: TransportPort + 'static> BACnetServer<T> {
    pub(super) async fn start_with_clock_mode_and_bindings(
        ...
        let dispatch_task = super::lifecycle_dispatch::spawn_dispatch_task(
            apdu_rx,
            Arc::clone(&network),
            Arc::clone(&db),
            Arc::clone(&cov_table),
            ...
            Arc::clone(&confirmed_request_tracker),
            Arc::clone(&device_bindings),
            Arc::clone(&comm_state),
            Arc::clone(&dcc_timer),
            Arc::clone(&dcc_outcomes),
            Arc::new(config.clone()),
            clock.clone(),
            Arc::clone(&discovery_limiter),
            Arc::clone(&request_tasks),
        );
